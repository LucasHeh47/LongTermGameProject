...
public class SpriteTools {
	...
	public static BufferedImage tintGrayscaleImage(BufferedImage source, Color tint) {
	    int width = source.getWidth();
	    int height = source.getHeight();
	    BufferedImage tintedImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
	    ...
	    for (int x = 0; x < width; x++) {
	        for (int y = 0; y < height; y++) {
	            ...
	            int intensity = (pixel >> 16) & 0xFF; // Red channel for grayscale (same as green and blue)

	            // Apply the tint proportionally to the intensity
	            int red = (tint.getRed() * intensity) / 255;
	            int green = (tint.getGreen() * intensity) / 255;
	            int blue = (tint.getBlue() * intensity) / 255;

	            // Use full alpha (255) or extract alpha if the source has it
	            int alpha = (pixel >> 24) & 0xFF;
	            if (alpha == 0) alpha = 255; // Assume fully opaque if no alpha channel exists

	            // Recombine the tinted color
	            int tintedPixel = (alpha << 24) | (red << 16) | (green << 8) | blue;
    ...
    public static BufferedImage setOpacity(BufferedImage source, float alpha) {
        int width = source.getWidth();
        int height = source.getHeight();
        ...
        // Apply the alpha composite for the desired opacity
        g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha));

        // Draw the original image with the applied alpha
